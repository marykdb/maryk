...
- `sql [options] SELECT ...` or `sql [options] --file <path>` — run a bounded read-only SQL query.
...
SQL runs against the connected store. The command preserves SQL string literals, quoted names, and comments:

```text
sql SELECT name FROM Person WHERE name LIKE 'A%' ORDER BY name LIMIT 20
sql SELECT category, COUNT(*), SUM(amount) FROM Invoice GROUP BY category
sql --file ./report.sql
maryk --connect rocksdb --dir ./data --exec "sql SELECT COUNT(*) FROM Invoice"
```

Options: `--snapshot`, `--to-version <n>`, `--include-deleted`, and `--no-table-scan`.
Choose at most one consistency option (`--snapshot` or `--to-version`). Use `--` before SQL that starts with a line comment, for example `sql -- -- report` followed by a newline and `SELECT 1`; a SQL file can start with comments directly.
Versioned reads require retained history and an authoritative snapshot provider. Table scans are enabled by default in the CLI, with at most 10,000 result rows and 8 MiB of logical buffered data. Query files are limited to 64 KiB. Output uses tab-separated columns, quotes strings, renders SQL `NULL` explicitly, and escapes control characters.

Parameters are available through the Kotlin SQL API. In the CLI, use SQL literals or a file. SQL does not expose writes or schema changes; continue using Maryk commands for those operations.
...
MARYK_BEARER_TOKEN=replace-with-a-secret maryk serve rocksdb --dir ./data --host 127.0.0.1 --bearer-token-env MARYK_BEARER_TOKEN
serve rocksdb --dir ./data --host 127.0.0.1 --bearer-token-file ./maryk-token
printf '%s' 'replace-with-a-secret' | maryk serve rocksdb --dir ./data --host 127.0.0.1 --bearer-token-stdin
```
...
host: 127.0.0.1
port: 8210
bearer-token-env: MARYK_BEARER_TOKEN
```
...
Use exactly one of `--bearer-token-env <name>`, `--bearer-token-file <path>`, or
`--bearer-token-stdin` (the same `bearer-token-env`, `bearer-token-file`, and
blank `bearer-token-stdin:` config keys are supported). `--bearer-token <token>`
remains available for compatibility but warns because command-line arguments can
be visible to other local users and process monitors.
...
- Non-loopback plaintext binds require the explicit unsafe `--allow-insecure-remote-binding` opt-in, even with `--bearer-token`.
...
The CLI migration command works with local RocksDB/FoundationDB stores. Remote
Store authorization and migration administration are available to programmatic
server integrations, but the CLI currently has no Remote migration-admin
connection path. Status includes phase, attempt, estimated completion time, and

...
open class ListReference<T: Any, CX: IsPropertyContext> (
        propertyDefinition: DataObjectListProperty<T, CX, *>,
        parentReference: CanHaveComplexChildReference<*, *, *>?
) : ValuePropertyReference<List<T>, DataObjectListProperty<T, CX, *>, CanHaveComplexChildReference<*, *, *>>(
        propertyDefinition,
        parentReference
), HasEmbeddedPropertyReference<T> {
    override fun getEmbedded(name: String) = when(name[0]) {
        '@' -> ListItemReference(name.substring(1).toInt(), propertyDefinition.property, this)
    ...
    override fun getEmbeddedRef(reader: () -> Byte): IsPropertyReference<*, IsPropertyDefinition<*>> {
        ...
        return when(index) {
            0 -> ListItemReference(initIntByVar(reader), propertyDefinition.property, this)

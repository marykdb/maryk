    ...
    when (filter.filterType) {
        FilterType.And -> {
            ...
            for (f in and.filters) {
                if (!filterMatches(key, f, transaction, columnFamilies, readOptions, toVersion)) return false
            }
            return true
        }
        FilterType.Or -> {
            val or = filter as Or
            for (f in or.filters) {
                if (filterMatches(key, f, transaction, columnFamilies, readOptions, toVersion)) return true
            }
            return false
        }
        FilterType.Not -> {
            ...
            for (aFilter in notFilter.filters) {
                ...
                if (filterMatches(key, aFilter, transaction, columnFamilies, readOptions, toVersion)) return false
            }
            return true
        }
        FilterType.Exists -> {
            ...
            for (propRef in exists.references) {
                if (!transaction.matchQualifier(columnFamilies, readOptions, key, propRef, toVersion) { it != null }) return false
            }
            return true
        }
        FilterType.Equals -> {
            ...
            for ((propRef, value) in equals.referenceValuePairs) {
                if (!transaction.matchQualifier(columnFamilies, readOptions, key, propRef, toVersion) { it == value }) return false
            }
            return true
        }
        FilterType.LessThan -> {
            ...
            for ((propRef, value) in lessThan.referenceValuePairs) {
                ...
                if (!transaction.matchQualifier(columnFamilies, readOptions, key, propRef, toVersion) { it != null && (value as Comparable<Any>) > it }) return false
            }
            return true
        }
        ...
        FilterType.Prefix -> {
            ...
            for ((propRef, prefix) in prefixFilter.referenceValuePairs) {
                if (!transaction.matchQualifier(columnFamilies, readOptions, key, propRef, toVersion) { it != null && it.startsWith(prefix) }) return false
            }
            return true
        }
        FilterType.Range -> {
            ...
            for ((propRef, range) in rangeFilter.referenceRangePairs) {
                @Suppress("UNCHECKED_CAST")
                if (!transaction.matchQualifier(columnFamilies, readOptions, key, propRef, toVersion) { it != null && (it as Comparable<Any>) in range as ValueRange<Comparable<Any>> }) return false
            }
            return true
        }
        FilterType.RegEx -> {
            ...
            for ((propRef, regEx) in regExFilter.referenceValuePairs) {
                if (!transaction.matchQualifier(columnFamilies, readOptions, key, propRef, toVersion) { it != null && regEx.matches(it) }) return false
            }
            return true
        }
        FilterType.ValueIn -> {
            ...
            for ((propRef, values) in valueInFilter.referenceValuePairs) {
                if (!transaction.matchQualifier(columnFamilies, readOptions, key, propRef, toVersion) { it != null && values.contains(it) }) return false
            }
            return true
        }
    }

...
class UpdateHistoryMalformedEntryTest {
    @Test
    fun scanUpdateHistorySkipsLongMalformedDuplicateEntry() = runTest {
        val folder = createTestDBFolder("update-history-malformed-duplicate")
        ...
        try {
            ...
            store.db.put(
                columnFamilies.updateHistory!!,
                changeStatus.version.toReversedVersionBytes() + addStatus.key.bytes + byteArrayOf(1),
                byteArrayOf()
            )

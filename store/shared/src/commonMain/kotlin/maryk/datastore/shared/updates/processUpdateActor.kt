...
internal fun <DM: IsRootValuesDataModel<P>, P: PropertyDefinitions> AbstractDataStore.processUpdateActor(hasStarted: CompletableDeferred<Unit>): SendChannel<IsUpdateAction> =
    BroadcastChannel<IsUpdateAction>(Channel.BUFFERED).also {
        this.launch {
            val updateListeners = mutableMapOf<UInt, MutableList<UpdateListener<*, *, *>>>()

            it.asFlow()
                .onStart { hasStarted.complete(Unit) }
                .onCompletion {

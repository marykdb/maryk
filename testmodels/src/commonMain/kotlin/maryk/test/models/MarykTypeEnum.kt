...
sealed class MarykTypeEnum<T: Any>(
    ...
    object T1: MarykTypeEnum<String>(1u,
        StringDefinition(regEx = "[^&]+"), setOf("Type1")
    )
    object T2: MarykTypeEnum<Int>(2u,
        NumberDefinition(type = SInt32, maxValue = 2000)
    )
    ...
    object T4: MarykTypeEnum<List<String>>(4u,
        ListDefinition(valueDefinition = StringDefinition(regEx = "[^&]+"))
    )
    object T5: MarykTypeEnum<Set<String>>(5u,
        SetDefinition(valueDefinition = StringDefinition(regEx = "[^&]+"))
    )
    object T6: MarykTypeEnum<Map<UInt, String>>(6u,
        MapDefinition(
            keyDefinition = NumberDefinition(type = UInt32),
            valueDefinition = StringDefinition(regEx = "[^&]+")
        )
    )
    object T7: MarykTypeEnum<TypedValue<SimpleMarykTypeEnum<out Any>, Any>>(7u,
        MultiTypeDefinition(typeEnum = SimpleMarykTypeEnum)
    )

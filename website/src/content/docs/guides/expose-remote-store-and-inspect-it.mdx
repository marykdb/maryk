...
Loopback serving needs no authentication. Non-loopback binds require a bearer
token (or the explicit unsafe `--allow-insecure-remote-binding` opt-in). Bearer
authentication does not encrypt traffic; terminate TLS or use SSH tunneling
outside localhost.
...
Deploy the remote client and server together before using batched execute
requests. Mixed execute-protocol generations are not a supported batch
compatibility target; legacy single-request framing is only a transition aid.

...
5. Use Memory Store in tests, then IndexedDB for browser-local data or RocksDB for durable embedded data.
...
## Persist in browsers with IndexedDB

Use IndexedDB for durable local storage in JS and WasmJS browser apps:

```kotlin
IndexedDbDataStore.open(
    databaseName = "maryk-local-app",
    keepAllVersions = true,
    keepUpdateHistoryIndex = true,
    dataModelsById = mapOf(
        1u to Project,
...
The IndexedDB store uses the browser's native IndexedDB database, key ranges, transactions, and cursor scans. It is the browser-local persistence path for web clients.
...
The request code stays the same across Memory, IndexedDB, and RocksDB.
...
Use the desktop App when you need a visual loop: models, records, filters, form-based edits, and history in one place.
